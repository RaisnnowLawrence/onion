...
RUN_ID="mme_dyfo_decision_6exp_20260626_122553"
...
LOGDIR="/data2/lizhengxue/WorkSpace/onion_output/ablation_summary/logs_mme_dyfo_decision_test/${RUN_ID}_retry_oom"
...
run_retry_shard() {
  ...
  local cache_dir="/data2/lizhengxue/WorkSpace/onion_output/mme/cache_${RUN_ID}_${exp_name}_retry_gpu${gpu_id}"
  local log_file="${LOGDIR}/${exp_name}_retry_shard${shard_id}_gpu${gpu_id}.log"
  local extra
  extra="$(extra_args_for_exp "${exp_name}")"

  mkdir -p "${out_dir}" "${cache_dir}"
  echo "[$(date '+%F %T')] retry exp=${exp_name} shard=${shard_id}/3 gpu=${gpu_id} log=${log_file}"
...
merge_exp() {
  ...
  local out_dir="${OUT_ROOT}/${ENGINE}_${exp_name}"
  local cache_dir="/data2/lizhengxue/WorkSpace/onion_output/mme/cache_${RUN_ID}_${exp_name}_retry_gpu${gpu_id}"
  local log_file="${LOGDIR}/${exp_name}_retry_merge.log"
  local extra
  extra="$(extra_args_for_exp "${exp_name}")"
...
run_one() {
  ...
  local shard_id="$2"
  local gpu_id="$3"
  run_retry_shard "${exp_name}" "${shard_id}" "${gpu_id}"
  merge_exp "${exp_name}" "${gpu_id}"
}

run_one dyfo_evidence_inject 2 1 &
run_one dyfo_weighted_vote 1 2 &
run_one dyfo_weighted_vote_more_search 2 3 &
run_one dyfo_weighted_vote_visual_update 2 4 &
wait
