...
WATCH_LOG="${REPORT_DIR}/mcts_narrow_round1_n10_6shards_merge_watch.log"
...
echo "[$(timestamp)] MCTS narrow 6-shard merge watcher started" | tee -a "${WATCH_LOG}"
while true; do
    if merge_if_complete; then
        break
    fi
    if [[ -z "$(running_pids)" ]]; then
        echo "[$(timestamp)] no running MCTS narrow 6-shard processes; current samples=$(sample_count)" | tee -a "${WATCH_LOG}"
    fi
    sleep 600
done
echo "[$(timestamp)] MCTS narrow 6-shard merge watcher finished" | tee -a "${WATCH_LOG}"
