...
RUN_ID="${1:-vqav2_qwen4b_dyfo_enhance_gpu23_6shards_3000_$(date +%Y%m%d_%H%M%S)}"
...
LOG_ROOT="/data2/lizhengxue/WorkSpace/onion_output/ablation_summary/logs_vqav2_qwen4b_dyfo_enhance"
EXP_OUT="${OUT_ROOT}/${RUN_ID}"
SUMMARY_LOG="${EXP_OUT}/summary.log"
NUM_SHARDS="${NUM_SHARDS:-6}"
MAX_SAMPLES_PER_SHARD="${MAX_SAMPLES_PER_SHARD:-500}"
...
run_shard() {
  ...
  echo "[launch] gpu=${gpu_id} shard=${shard_id}/${NUM_SHARDS} max_samples=${MAX_SAMPLES_PER_SHARD} log=${log_file}"
...
echo "[run] target_samples=$((NUM_SHARDS * MAX_SAMPLES_PER_SHARD))"
echo "[run] assignment: gpu2=shards0,1,2 gpu3=shards3,4,5"
echo "[run] dyfo enhancement: --mcts_action_mode dyfo_evidence --dyfo_decision_mode evidence_inject --dyfo_n_simulations 6 --dyfo_max_depth 3"
