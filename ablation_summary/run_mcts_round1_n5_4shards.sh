...
ENGINE=qwen3-VL-4B
EXP_NAME=mcts_image_no_cot_rounds1_n5_4shards
...
LOG_DIR="${REPORT_DIR}/logs_mcts_round1_n5_4shards"
MASTER_LOG="${REPORT_DIR}/mcts_round1_n5_4shards_master.log"

CACHE_PATH=/data2/lizhengxue/WorkSpace/huchunning/VisualCoT-data/image_cache_onion/cache_forward4b_mcts_round1_n5_4shards
...
launch_shard() {
    ...
    echo "[$(timestamp)] launch ${EXP_NAME} shard ${shard}/4 on GPU ${gpu}" | tee -a "${MASTER_LOG}"
...
main() {
    echo "[$(timestamp)] MCTS round1 4-shard experiment started" | tee -a "${MASTER_LOG}"
    echo "[$(timestamp)] output: ${OUTPUT_PATH}" | tee -a "${MASTER_LOG}"

    local pid0 pid1 pid2 pid3
    launch_shard 0 0; pid0="${RUN_PID}"
    launch_shard 1 1; pid1="${RUN_PID}"
    launch_shard 2 6; pid2="${RUN_PID}"
    launch_shard 3 6; pid3="${RUN_PID}"
    echo "[$(timestamp)] pids: shard0=${pid0}, shard1=${pid1}, shard2=${pid2}, shard3=${pid3}" | tee -a "${MASTER_LOG}"
    ...
    echo "[$(timestamp)] MCTS round1 4-shard experiment finished with $(sample_count)/1145 samples" | tee -a "${MASTER_LOG}"
}
