...
DECISION_MODE="$1"
GPU_ID="$2"
RUN_ID="${3:-vqav2_qwen4b_dyfo_${DECISION_MODE}_gpu${GPU_ID}_4shards_3000_$(date +%Y%m%d_%H%M%S)}"
...
LOG_ROOT="/data2/lizhengxue/WorkSpace/onion_output/ablation_summary/logs_vqav2_qwen4b_dyfo_decision"
EXP_OUT="${OUT_ROOT}/${RUN_ID}"
SUMMARY_LOG="${EXP_OUT}/summary.log"
NUM_SHARDS="${NUM_SHARDS:-4}"
MAX_SAMPLES_PER_SHARD="${MAX_SAMPLES_PER_SHARD:-750}"
...
run_shard() {
  ...
  echo "[launch] gpu=${GPU_ID} shard=${shard_id}/${NUM_SHARDS} max_samples=${MAX_SAMPLES_PER_SHARD} mode=${DECISION_MODE} log=${log_file}"
