...
LOG_DIR=/data2/lizhengxue/WorkSpace/onion_output/ablation_summary/logs_reviewer_evidence_selective_3shards_gpu456
NAME=reviewer_evidence_selective_rounds1_3shards_gpu456
...
run_shard() {
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${PY}" "${BASE}/forward_code/onion.py" \
    ...
    > "${LOG_DIR}/${NAME}_shard${shard}_gpu${gpu}.log" 2>&1
}
...
echo "selective reviewer evidence gpu456 finished: ${OUT}"
