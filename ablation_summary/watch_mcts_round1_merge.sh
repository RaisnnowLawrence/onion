...
WATCH_LOG="${REPORT_DIR}/mcts_round1_merge_watch.log"
...
echo "[$(timestamp)] MCTS merge watcher started" | tee -a "${WATCH_LOG}"
while true; do
    if merge_if_complete; then
        break
    fi
    if [[ -z "$(running_pids)" ]]; then
        echo "[$(timestamp)] no running MCTS shard processes; current samples=$(sample_count)" | tee -a "${WATCH_LOG}"
    fi
    sleep 600
done
echo "[$(timestamp)] MCTS merge watcher finished" | tee -a "${WATCH_LOG}"
