...
RUN_ID="${1:-okvqa_knowledge_notes_$(date +%Y%m%d_%H%M%S)}"
REPO_DIR="/data2/lizhengxue/WorkSpace/onion"
PYTHON="/data2/lizhengxue/anaconda3/envs/sam/bin/python"
ROOT_OUT="/data2/lizhengxue/WorkSpace/onion_output/okvqa/knowledge_notes_ablation/${RUN_ID}"
LOGDIR="/data2/lizhengxue/WorkSpace/onion_output/ablation_summary/logs_okvqa_knowledge_notes/${RUN_ID}"
...
COMMON_ARGS=(
  ...
  --knowledge_use_wit
  --knowledge_enhance_trigger knowledge_qtype
  --knowledge_top_k 5
)
...
for shard_id in 0 1 2; do
  launch_one knowledge_raw_wit 5 "${shard_id}" --knowledge_notes_mode raw_retrieved
  launch_one knowledge_notes_wit 6 "${shard_id}" --knowledge_notes_mode notes --knowledge_notes_use_image
  if [[ "${shard_id}" != "2" ]]; then
