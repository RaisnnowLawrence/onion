...
RUN_ID="${1:-gqa_testdev_dyfo_clip_statement_full_12shards_$(date +%Y%m%d_%H%M%S)}"
  ...
  while true; do
    ...
    if [[ "${#selected[@]}" -eq 4 ]]; then
      ...
      for gpu in "${selected[@]}"; do
        ...
        if [[ "$recheck" -lt "$MIN_FREE_MB" ]]; then
          valid=0
        fi
      done
      if [[ "$valid" -eq 1 ]]; then
        SELECTED_GPUS=("${selected[@]}")
        echo "[gpu-wait] acquired four GPUs: ${SELECTED_GPUS[*]}"
        return 0
      fi
    fi
    release_locks
    echo "[gpu-wait] need four GPUs with free_mb>=${MIN_FREE_MB}; candidates=${CANDIDATE_GPUS}; sleep=${GPU_WAIT_SECONDS}s"
...
on_term() {
  echo "[signal] stopping four GPU runners"
  stop_runners
  release_locks
  exit 143
}
...
COMMON_ENV=(
  NUM_SHARDS=12
  SPLIT_NAME=testdev
  DYFO_DECISION_MODE=clip_statement_override
  ...
  DYFO_CLIP_STATEMENT_MARGIN=0.0
  DYFO_CLIP_STATEMENT_FOCUS_GAIN=0.0
  SHARD_LAUNCH_DELAY="$SHARD_LAUNCH_DELAY"
  MERGE_ON_DONE=0
)

echo "[run] RUN_ID=${RUN_ID}"
echo "[run] full GQA testdev; qwen3-VL-4B generator; CLIP statement verifier"
echo "[run] multimodal focus rewrite=1 padding=1.2; four GPUs x three shards"

acquire_four_gpus

SHARD_GROUPS=("0 4 8" "1 5 9" "2 6 10" "3 7 11")
