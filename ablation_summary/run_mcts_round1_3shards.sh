...
ENGINE=qwen3-VL-4B
EXP_NAME=mcts_image_no_cot_rounds1
...
LOG_DIR="${REPORT_DIR}/logs_mcts_round1"
MASTER_LOG="${REPORT_DIR}/mcts_round1_master.log"

CACHE_PATH=/data2/lizhengxue/WorkSpace/huchunning/VisualCoT-data/image_cache_onion/cache_forward4b_mcts_round1
...
launch_shard() {
    ...
    echo "[$(timestamp)] launch ${EXP_NAME} shard ${shard}/3 on GPU ${gpu}" | tee -a "${MASTER_LOG}"
...
main() {
    echo "[$(timestamp)] MCTS round1 3-shard experiment started" | tee -a "${MASTER_LOG}"
    ...
    launch_shard 2 6
    pid2="${RUN_PID}"
    echo "[$(timestamp)] pids: shard0=${pid0}, shard1=${pid1}, shard2=${pid2}" | tee -a "${MASTER_LOG}"
    ...
    echo "[$(timestamp)] MCTS round1 3-shard experiment finished with $(sample_count)/1145 samples" | tee -a "${MASTER_LOG}"
}
