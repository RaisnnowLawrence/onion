...
RUN_BATCH_ID="${RUN_BATCH_ID:-gqa_testdev_dyfo_token_confidence_full_wait2_$(date +%Y%m%d_%H%M%S)}"
RUN_ID="${RUN_ID:-gqa_testdev_dyfo_token_confidence_full_6shards_${RUN_BATCH_ID}}"
...
job_id=$(
  "$TS_BIN" -L wait_two_gpus_full env \
...
echo "[queue] waits for two GPUs among 0-7 with at least 40960 MiB free each"
"$TS_BIN" -l
