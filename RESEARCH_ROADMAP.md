...
### 5. NoteMR-Inspired Knowledge Notes

目标：把当前 onion 的弱 `knowledge_enhance` 升级为更论文友好的两阶段 Knowledge Notes 机制，优先服务 OK-VQA 和 A-OKVQA 中的知识型、常识桥接型困难题。

背景：

- `/data2/lizhengxue/WorkSpace/NoteMR-main` 的核心思想是：先检索外部知识，再用 MLLM 根据图像过滤和压缩检索知识，生成 Knowledge Notes，最后用 `原图 + Visual Notes + Knowledge Notes + 问题` 回答。
- 当前 onion 的 `knowledge_enhance` 主要是让 Qwen-VL 基于图像和问题临时生成常识补充，没有真正接外部知识库，也没有显式检索-过滤过程。
- 因此 NoteMR 方向比单纯 prompt engineering 更适合作为论文主线：它能把“知识从哪里来、如何过滤、如何注入回答”讲清楚。

推荐迁移结构：

```text
question + image -> retrieve top-k external knowledge
question + image + top-k knowledge -> generate concise Knowledge Notes
question + image/caption + Knowledge Notes -> final short answer
```

与当前 onion 模块的关系：

- `direct` 仍然作为强基本盘，不应被 notes 方法替换。
- `knowledge_notes` 应作为 evidence / candidate generator，重点提升知识型 hard subset 和 oracle。
- `raw retrieved knowledge` 与 `knowledge notes` 必须分开消融，验证 notes 过滤是否真的减少噪声。
- `visual notes` 可以暂缓；我们已有 MCTS/SAM image enhancement，可先只迁移 Knowledge Notes。

NoteMR 工程缺失与影响：

- 缺少 FLMR/PreFLMR 检索代码：仓库只假设已有 `retriever_test.json`。这影响直接复现，但不影响我们自建轻量 RAG。
- 缺少 GradCAM / Visual Notes 生成代码：`generate_output.py` 直接读取预生成 heatmap 图。短期不迁移 Visual Notes。
- 路径和模型硬编码严重：大量 `/data/fangwenlong/...` 需要参数化后才能在本机运行。

可补充方案：

- 短期：使用 BM25 / sentence-transformer / 现有知识文件做轻量检索，先验证 Knowledge Notes 是否提升。
- 中期：引入更强的多模态检索，例如 CLIP/FLMR-style image-question retrieval。
- 长期：若 Knowledge Notes 有收益，再考虑 GradCAM/ROI visual notes，或者复用 onion 现有 MCTS/SAM 作为 visual notes 替代。

推荐消融：

1. `direct_baseline`
2. `raw_retrieved_knowledge_direct_injection`
3. `retrieval_free_notes`
4. `bm25_knowledge_notes`
5. `knowledge_notes_plus_caption`
6. `knowledge_notes_plus_image_enhance`

每个实验需要报告：

- 全量 val accuracy。
- 知识型问题子集 accuracy。
- old9 all-wrong / latest all-wrong hard subset rescue cases。
- oracle 增益。
- direct-correct damage count。

保留标准：

- 如果 `knowledge_notes` 单策略不高，但能显著增加 hard subset positive cases 或 oracle，可保留为候选生成模块。
- 如果 `raw knowledge` 降低准确率而 `knowledge notes` 提升，说明 notes 过滤有论文价值。
- 如果 notes 主要伤害 direct-correct 样本，则必须接 conservative arbitration，而不是作为默认注入。
...
### Priority 6: NoteMR-Inspired Knowledge Notes Pilot

目标：先用最小成本验证 Knowledge Notes 是否值得成为主线。

第一阶段建议：

1. 为 OK-VQA val 构造轻量 top-k knowledge retrieval 文件。
2. 新增 `--use_knowledge_notes`，生成并缓存每题的 Knowledge Notes。
3. 新增 `--knowledge_notes_path`，最终回答时读取 notes 并注入 context。
4. 跑三组对照：
   - direct baseline
   - raw retrieved knowledge injection
   - generated Knowledge Notes injection

第二阶段建议：

1. 只在知识型问题或 hard subset 上触发 notes。
2. 将 Knowledge Notes 作为 candidate generator，而不是默认覆盖 direct。
3. 与 `caption_only`、`knowledge_only`、`all_three` 做统一 oracle 分析。

短期成功标准：

```text
OK-VQA val full accuracy 不低于 direct baseline
或 hard subset / knowledge subset 有明确 rescue 增益
或 oracle 明显上升
```
...
```text
Knowledge Notes > raw retrieved knowledge
Knowledge Notes 对 hard subset 有稳定 positive cases
与 strong direct + conservative arbitration 组合后实现净提升
```
...
OK-VQA / A-OKVQA 中一部分困难题需要外部知识，但直接注入原始检索知识容易带来噪声。`retrieval -> Knowledge Notes -> answer` 可能比当前 onion 的模型自发知识补充更稳定，尤其适合作为知识型 hard subset 的候选生成模块。
